...
const OnlyUsers = () => {
    ...
    useEffect(() => {
        ...
        axios.get('https://adbackend.tourglobalhub.com/api/users')
            .then(response => {
                ...
                setFilteredUsers(response.data.filter(user => user.role === 'user'));
            })
        ...
        if (isEditMode) {
            ...
            axios.put(`https://adbackend.tourglobalhub.com/api/users/${formData.id}`, formData)
                .then(response => {
                    ...
                    setFilteredUsers(filteredUsers.map(user => (user.id === formData.id ? response.data.user : user)));
                    hideDialog();
                })
    ...
    return (
        <div className="p-6">
            ...
            <DataTable value={filteredUsers} paginator rows={10} className="p-datatable-gridlines">
